<template>
    <div class="wrapper">
        <main>
            ...
            <div class="table__container p-4 pt-3 rounded">
                <div class="d-flex justify-content-between mt-2 mb-4">
                    <h4>Issued Books</h4>
                    <div>
                        <b-button v-if="selectedRow[0] && selectedIssuedBook.ib_status == 'active'"
                            v-b-modal.updateIssuedBookModal class="mr-2" variant="primary">Update</b-button>
                    </div>
                </div>
                ...
                <b-table :items="items" :per-page="perPage" :fields="fields" :current-page="currentPage"
                    ...
                    <template #cell(ib_status)="row">
                        <b-badge pill v-if="row.item.ib_status == 'Overdue'" class="bg-danger">{{
                            row.item.ib_status
                        }}</b-badge>
        ...
        <AppModal modalId="updateIssuedBookModal" hideFooter :key="modalKey">
            ...
            <template #modal-body>
                <form class="px-2" @submit.prevent="editIssuedBook(selectedIssuedBook.issue_id, selectedIssuedBook)">
                    <div class="row mb-2">
                        <label for="date_returned">Date Returned</label>
                        <b-form-input v-model="selectedIssuedBook.date_returned" type="date"
                            id="date_returned"></b-form-input>
                    </div>
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: 'reader_name', thStyle: { textTransform: "uppercase", width: '190px' }, sortable: true },
                {
                ...
                { key: 'ib_status', label: 'status', thStyle: { textTransform: "uppercase", width: '110px' } },
            ],
            perPage: 12,
            currentPage: 1,
            totalRows: 1,
    ...
    methods: {
        ...
        getSelectedBookData() {
            this.selectedBookData = this.books.books.find(b => b.isbn == this.selectedIssuedBook.b_isbn)
        },
        editIssuedBook(id, issuedBook) {
            this.$store.dispatch('editIssuedBook', { id, issuedBook })
                .then(() => {
                    this.rerenderModal();
                    this.$router.go(0)
                })
                .catch(() => {
                    console.log('There was a problem adding the issuedBook.')
                })
